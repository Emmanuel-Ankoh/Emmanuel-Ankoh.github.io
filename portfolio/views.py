...
def home(request):
	"""Render the homepage with a simple cache and a preview bypass.

	Use `?preview=1` (or `?nocache=1`) in the querystring to bypass the
	cached version and render live content from the database. This is useful
	when previewing admin edits without waiting for the cache to expire.
	"""
	from django.core.cache import cache

	# Preview param bypasses cache for immediate admin/content previewing.
	# Restrict bypass to staff users (or when DEBUG=True) to avoid public cache
	# bypassing in production. Use ?preview=1 or ?nocache=1 in the querystring.
	raw_preview = bool(request.GET.get('preview') or request.GET.get('nocache'))
	preview = False
	try:
		if settings.DEBUG:
			preview = raw_preview
		else:
			# Only allow preview when the requesting user is an authenticated staff
			# member to prevent public cache bypassing.
			if raw_preview and hasattr(request, 'user') and request.user.is_authenticated and request.user.is_staff:
				preview = True
	except Exception:
		preview = False
	...
	# Expose preview flag to templates so admin users can see a small debug banner
	context['preview'] = preview

	# Render to string so we can cache the HTML and still bypass when requested
	html = render_to_string('home.html', context, request=request)
	if not preview:
		cache.set(cache_key, html, HOME_CACHE_SECONDS)
...
def server_error(request, template_name='500.html'):
	"""Custom 500 handler that prefers a minimal static fallback template.

	Try rendering a completely static `500_fallback.html` without running
	context processors (so it doesn't touch the DB). If that fails, fall
	back to the normal template rendering with a tiny safe context, and
	finally return a hard-coded inline HTML string as the last resort.
	"""
	# Try the static fallback first using a plain Context (no processors)
	try:
		from django.template import Context
		from django.template.loader import get_template

		tmpl = get_template('500_fallback.html')
		html = tmpl.render(Context({}))
		return HttpResponse(html, status=500, content_type='text/html')
	except Exception:
		# If the static fallback is unavailable or fails, continue to safer
		# rendering attempts that supply a minimal context.
		pass

	ctx = {
		'PROFILE': getattr(settings, 'PROFILE', {}),
		# Provide SITE dict if your templates expect SITE.* values
		'SITE': getattr(settings, 'SITE', {}),
	}
	try:
		return render(request, template_name, ctx, status=500)
	except Exception:
		# Final inline fallback that does not rely on templates or context
		html = (
			'<!doctype html><html><head><meta charset="utf-8"><title>500 — Server Error</title>'
			'<meta name="viewport" content="width=device-width,initial-scale=1"></head><body style="font-family:system-ui,-apple-system,Segoe UI,Roboto,Helvetica,Arial;max-width:640px;margin:48px auto;padding:24px;text-align:center;">'
			'<h1 style="font-size:28px;margin-bottom:12px;">500 — Something went wrong</h1>'
			'<p style="color:#333;margin-bottom:18px;">An unexpected error occurred. Please try again later.</p>'
			'<p><a href="/" style="display:inline-block;margin-right:8px;padding:10px 14px;background:#111;color:#fff;text-decoration:none;border-radius:6px;">Home</a>'
			'<a href="/contact/" style="display:inline-block;padding:10px 14px;border:1px solid #ccc;border-radius:6px;color:#111;text-decoration:none;">Contact</a></p>'
			'</body></html>'
		)
		return HttpResponse(html, status=500, content_type='text/html')
...
def about_pdf(request):
	...
	try:
		...
		try:
			...
			style_files = []
			# Prefer a dedicated print stylesheet for PDF generation to avoid
			# parsing the full site stylesheet (which contains modern CSS
			# features WeasyPrint will warn about). If `print.css` exists,
			# use it alone; otherwise fall back to `styles.css` if present.
			print_path = css_dir / 'print.css'
			styles_path = css_dir / 'styles.css'
			if print_path.exists():
				style_files.append(weasyprint.CSS(filename=str(print_path)))
			elif styles_path.exists():
				style_files.append(weasyprint.CSS(filename=str(styles_path)))
...
def gallery(request):
	...
	def _safe_image_info(field):
		"""Return a dict with url, width, height for an ImageField-like object.

		Accessing `.width`/`.height` may attempt to open the underlying file and
		raise FileNotFoundError on hosts where media files are missing. Wrap
		those accesses in try/except and return None for dimensions when not
		available.
		"""
		if not field:
			return {'url': '', 'width': None, 'height': None}
		info = {'url': '', 'width': None, 'height': None}
		try:
			# url property usually does not open the file; safe to fetch
			info['url'] = getattr(field, 'url', '') or ''
		except Exception:
			info['url'] = ''
		try:
			# width/height access may open the file; guard it
			info['width'] = getattr(field, 'width', None)
		except Exception:
			info['width'] = None
		try:
			info['height'] = getattr(field, 'height', None)
		except Exception:
			info['height'] = None
		return info
	...
	if src in ('all', 'projects'):
		...
		for p in proj_qs:
			img_info = _safe_image_info(p.image)
			items.append({
				'title': p.title,
	...
	if src in ('all', 'blog'):
		...
		for b in post_qs:
			img_info = _safe_image_info(b.thumbnail)
			items.append({
				'title': b.title,
	...
	if src in ('all', 'custom'):
		...
		for g in gqs:
			img_info = _safe_image_info(g.image)
			items.append({
				'title': g.title,
				'url': g.link_url() or '#',
				'img': img_info['url'],
